<%/*
       * Created on 26-giu-2006
       *
       * Copyright (c) EldaSoft S.p.A.
       * Tutti i diritti sono riservati.
       *
       * Questo codice sorgente e' materiale confidenziale di proprieta' di EldaSoft S.p.A.
       * In quanto tale non puo' essere distribuito liberamente ne' utilizzato a meno di 
       * aver prima formalizzato un accordo specifico con EldaSoft.
       */

      // PAGINA CHE CONTIENE L'ISTANZA DELLA SOTTOPARTE DELLA PAGINA DI LISTA GRUPPI 
      // CONTENENTE LE AZIONI DI CONTESTO
    %>

<%@ taglib uri="http://java.sun.com/jsp/jstl/functions" prefix="fn"%>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c"%>
<%@ taglib uri="http://www.eldasoft.it/genetags" prefix="gene"%>

<c:set var="listaOpzioniDisponibili" value="${fn:join(opzDisponibili,'#')}#"/>
<c:set var="listaOpzioniUtenteAbilitate" value="${fn:join(profiloUtente.funzioniUtenteAbilitate,'#')}#" />

<gene:template file="menuAzioni-template.jsp">
	<gene:insert name="addHistory">
		<gene:historyAdd titolo='Lista Utenti' id="lista" />
	</gene:insert>
</gene:template>

		<tr>
			<td class="titolomenulaterale">Lista: Azioni</td>
		</tr>
		<c:if test='${fn:contains(listaOpzioniDisponibili, "OP101#") && (fn:contains(listaOpzioniUtenteAbilitate, "ou11#") && !fn:contains(listaOpzioniUtenteAbilitate, "ou12#"))}'>
			<c:if test='${gene:checkProt(pageContext, "FUNZ.VIS.DEL.GENE.USRSYS-Lista.LISTADELSEL")}'>
			<tr>
				<td class="vocemenulaterale">
					<a href="javascript:eliminaSelez();" title="Elimina dati selezionati" tabindex="1500">Elimina dati selez.</a></td>
			</tr>
			</c:if>
			<tr>
				<td>&nbsp;</td>
			</tr>
			<tr>
				<td class="titolomenulaterale">Gestione Utenti</td>
			</tr>
			<c:if test='${gene:checkProt(pageContext, "FUNZ.VIS.INS.GENE.USRSYS-Lista.LISTANUOVO")}'>
			<tr>
				<td class="vocemenulaterale">
					<a href="javascript:schedaNuovo();" title="Crea nuovo utente" tabindex="1501">Nuovo</a></td>
			</tr>
			</c:if>
		</c:if>		
		<tr>
			<td>&nbsp;</td>
		</tr>
<jsp:include page="/WEB-INF/pages/commons/torna.jsp" />
		