...
<table class="dettaglio-tab-lista">
	<tr>
		<td>
			<gene:formLista entita="IMPCASE" where="IMPCASE.CODIMP = #IMPR.CODIMP#"  sortColumn="2" pagesize="20" tableclass="datilista" gestisciProtezioni="true" > 
				...
				<% // Campi veri e propri %>
				<gene:campoLista campo="NUMCOM" visibile="false" />
				<gene:campoLista campo="TIPCOM" headerClass="sortable" href="javascript:chiaveRiga='${chiaveRigaJava}';listaVisualizza();"/>
				<gene:campoLista campo="DATRIC" headerClass="sortable"/>
				<gene:campoLista campo="DATESI" headerClass="sortable" />
				<gene:campoLista campo="TIPESI" headerClass="sortable" />
			</gene:formLista>

			<c:if test='${gene:checkProtObj(pageContext, "MASC.VIS", "GENE.ImpcaseScheda"  )}' >
				<gene:PopUpItemResource variableJs="rigaPopUpMenu" resource="popupmenu.tags.lista.visualizza" title="Visualizza comunicazione"/>
			</c:if>
			<c:if test='${gene:checkProtFunz(pageContext, "MOD", "MOD"  )}' >
				<gene:PopUpItemResource variableJs="rigaPopUpMenu" resource="popupmenu.tags.lista.modifica" title="Modifica comunicazione" />
			</c:if>
			<c:if test='${gene:checkProtFunz(pageContext, "DEL", "DEL"  )}' >
				<gene:PopUpItemResource variableJs="rigaPopUpMenu" resource="popupmenu.tags.lista.elimina" title="Elimina comunicazione" />
			</c:if>
		</td>
	</tr>
	<tr>
