...
<tiles:insert definition=".dettaglioNoTabDef" flush="true">
	...
	<tiles:put name="head" type="string">
		...
		<!--
			function gestisciSubmit() {
				var continua = true;
				if (continua && document.cfgRichiestaAssistenzaForm.password.value != document.cfgRichiestaAssistenzaForm.newPassword.value) {
					alert("Nuova password e conferma nuova password non coincidono");
					continua = false;
				}
				if (continua && document.cfgRichiestaAssistenzaForm.password.value == document.cfgRichiestaAssistenzaForm.oldPassword.value) {
			...
			function annulla() {
				document.location.href = 'ConfigurazioneRichiestaAssistenza.do?'+csrfToken+'&metodo=visualizza';
			}
		-->
		</script>	</tiles:put>
	...
	<tiles:put name="titoloMaschera" type="string" value="Configurazione richiesta assistenza: impostazione password" />

	<tiles:put name="dettaglio" type="string">
		<form name="cfgRichiestaAssistenzaForm" action="${pageContext.request.contextPath}/ImpostaPasswordConfigurazioneRichiestaAssistenza.do" method="post">
