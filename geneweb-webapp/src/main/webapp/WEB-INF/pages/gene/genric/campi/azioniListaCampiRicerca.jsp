...
<c:set var="titoloHistory" value=" - ${nomeOggetto}"/>
	...
	<gene:insert name="addHistory">
		...
		<tr>
			<td class="vocemenulaterale">
				<a href="javascript:aggiungiCampi();" tabindex="1500" title="Aggiungi campi">Aggiungi campi</a>
			</td>
		</tr>	
		<tr>
			<td class="vocemenulaterale">
				<a href="javascript:trovaCampo();" tabindex="1501" title="Trova campo">Trova campo</a>
			</td>
		</tr>	
		<tr>
			<td class="vocemenulaterale">
				<a href="javascript:gestisciSubmit('elimina');" title="Elimina dati selezionati" tabindex="1502">Elimina dati selez.</a></td>
		</tr>
		<c:if test="${!empty (sessionScope.recordDettModificato)}">
		<tr>
			<td class="vocemenulaterale">
				<a href="javascript:salvaRicerca();" tabindex="1503" title="Salva report nella banca dati">Salva report</a>
			</td>
		</tr>
		<c:if test="${!empty (sessionScope.recordDettRicerca.testata.id)}">
		<tr>
			<td class="vocemenulaterale">
				<a href="javascript:ripristinaRicercaSalvata();" tabindex="1504" title="Annulla le modifiche e ricarica il report dalla banca dati">Annulla modifica</a>
			</td>
		</tr>		
		</c:if>
		<c:if test="${empty (sessionScope.recordDettRicerca.testata.id)}">
		<tr>
			<td class="vocemenulaterale">
				<a href="javascript:annullaCreazioneRicerca();" tabindex="1505" title="Annulla inserimento">Annulla inserimento</a>
			</td>
		</tr>		
		</c:if>
		</c:if>		
		...
		<tr>
			<td class="vocemenulaterale">
				<a href="javascript:eseguiRicerca();" tabindex="1506" title="Esegui estrazione report">Esegui report</a>
			</td>
		</tr>
		</c:if>
		<c:if test="${empty (sessionScope.recordDettModificato)}">
