 ...
 */
public class AddElencoCampiRicercaAction extends AbstractActionBaseGenRicerche {

  /** Logger Log4J di classe */
  static Logger               logger                   = Logger.getLogger(AddElencoCampiRicercaAction.class);
    ...
    if (contenitore.getNumeroCampi() + elencoCampi.length > 100) {
      ...
      for (int i = 0; i < elencoCampi.length; i++) {
        ...
        nomeTabellaUnivoco = elencoCampi[i].substring(0,
            elencoCampi[i].indexOf(CostantiGenRicerche.SEPARATORE_SCHEMA_TABELLA_CAMPO));
        mnemonicoCampo = elencoCampi[i].substring(elencoCampi[i].indexOf(
            CostantiGenRicerche.SEPARATORE_SCHEMA_TABELLA_CAMPO) + 1);
