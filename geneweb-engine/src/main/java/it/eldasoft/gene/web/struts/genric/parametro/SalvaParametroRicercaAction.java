 ...
 */
public class SalvaParametroRicercaAction extends
    AbstractDispatchActionBaseGenRicerche {

  private static final String SUCCESS_MODIFICA_FORWARD = "successUpdate";

  /** Logger Log4J di classe */
  static Logger               logger                   = Logger.getLogger(SalvaParametroRicercaAction.class);
  ...
  public ActionForward insert(ActionMapping mapping, ActionForm form,
    ...
    ParametroRicercaForm parametroRicercaForm = new ParametroRicercaForm();
    parametroRicercaForm.setCodiceParametro(CostantiGenRicerche.CODICE_PARAMETRO
        + (contenitore.getMaxCodiceParametro() + 1));
    parametroRicercaForm.setNome("Inserire "
        + parametroRicercaForm.getCodiceParametro());

    if (campo.getCodiceTabellato() != null) {
      parametroRicercaForm.setTipoParametro("T"); // valore copiato da TAB3 (G_x01)
      parametroRicercaForm.setTabCod(campo.getCodiceTabellato());
    } else {
      switch (campo.getTipoColonna()) {
      case Campo.TIPO_DATA:
        parametroRicercaForm.setTipoParametro("D"); // valore copiato da TAB3 (G_x01)
        break;
      case Campo.TIPO_DECIMALE:
        parametroRicercaForm.setTipoParametro("F"); // valore copiato da TAB3 (G_x01)
        break;
      case Campo.TIPO_INTERO:
        parametroRicercaForm.setTipoParametro("I");// valore copiato da TAB3 (G_x01)
        break;
      case Campo.TIPO_STRINGA:
        parametroRicercaForm.setTipoParametro("S");// valore copiato da TAB3 (G_x01)
        break;
    ...
    // Messaggio informativo: si ritorna alla lista filtri, mostrando il
    // messaggio che ricorda la possibilita' di personalizzare il parametro
    // appena
    // inserito nella condizione di filtro
    String messageKey = "info.genRic.inserimentoNuovoParametro";
  ...
  public ActionForward modifica(ActionMapping mapping, ActionForm form,
    ...
    // target di default
    String target = SUCCESS_MODIFICA_FORWARD;
