...
import it.eldasoft.gene.web.struts.genric.ordinamento.InitAddOrdinamentoRicercaAction;
...
/**
 * Ridefinizione della Action InitAddOrdinamentoRicercaAction per la gestione
 * degli ordinamenti nelle ricerche base
 *
 * @author Luca Giacomazzo
 */
public class InitAddOrdinamentoRicercaBaseAction extends InitAddOrdinamentoRicercaAction {

  private static final String ERROR_ADD_FORWARD = "errorInitAdd";

  /** Logger Log4J di classe */
  static Logger logger = Logger.getLogger(InitAddOrdinamentoRicercaBaseAction.class);
  ...
  protected ActionForward runAction(ActionMapping mapping, ActionForm form,
    ...
    if (continua && contenitore.getNumeroCampi() == 0) {
      ...
      // per la ricerca
      target = InitAddOrdinamentoRicercaBaseAction.ERROR_ADD_FORWARD;
    ...
    if (continua) {
      ...
      //for (int i = 0; i < elencoTabelleForm.size(); i++) {
        tabellaForm = (TabellaRicercaForm) elencoTabelleForm.elementAt(0);
