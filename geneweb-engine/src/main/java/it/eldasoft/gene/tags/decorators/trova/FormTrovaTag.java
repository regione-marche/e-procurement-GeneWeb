...
public class FormTrovaTag extends TagSupportGene {
  ...
  private HashMap            campiSaved                 = null;
  ...
  public int doStartTag() throws JspException {
    ...
    this.pageContext.setAttribute(UtilityTags.REQUEST_VAR_TIPO_PAGINA,
        UtilityTags.PAGINA_TROVA, PageContext.REQUEST_SCOPE);
    this.campo = 0;
    // Estraggo i valori dei campi salvati precedentemente
    if (this.pageContext.getSession().getAttribute(
        TrovaAction.SESSION_PENDICE_TROVA + this.getEntita()) instanceof HashMap)
      this.campiSaved = (HashMap) this.pageContext.getSession().getAttribute(
          TrovaAction.SESSION_PENDICE_TROVA + this.getEntita());
    else
      this.campiSaved = null;
    // Se non settata la lista allora setto la lista di default
    if (this.lista == null)
      this.setLista(UtilityTags.getPathFromEntita(this.getEntita())
          + "lista.jsp");
    JspWriter out = this.pageContext.getOut();
    try {
      out.println("<form name=\"trova\" action=\""
          + this.getContextPath()
      ...
      pageContext.setAttribute(UtilityTags.DEFAULT_HIDDEN_FORM_TO_JSP,
          this.getLista(), PageContext.REQUEST_SCOPE);
      ...
      out.print(UtilityTags.getHtmlHideInput(CAMPO_FILTRO, this.getFiltro()));
      ...
      this.getJavascript().println(
          this.getFormName() + "=new FormObj(document.trova);");
    } catch (IOException e) {
  ...
  public int doEndTag() throws JspException {
    ...
    try {
      ...
      if (valPrec == null || valPrec.length() == 0) valPrec = "20";
      String risultati[] = UtilityTags.stringToArray(UtilityTags.getResource(
          "label.tags.template.trova.numeroRisultati.valori", null, false), ';');

      for (int i = 0; i < risultati.length - 1; i += 2) {
        out.println("<option value=\""
            + risultati[i]
            + "\" "
            + (valPrec.equals(risultati[i]) ? "selected " : "")
            + ">"
            + risultati[i + 1]
            + "</option>\n");
      }
      out.println("</select>&nbsp;");
      out.print(UtilityTags.getResource(
          "label.tags.template.trova.numeroRisultati", null, true));
      out.println("</div>");
      if(((Boolean)this.pageContext.getServletContext().getAttribute(
          CostantiGenerali.ATTR_ATTIVA_CASE_SENSITIVE)).booleanValue()){
        ...
        out.print("/>&nbsp;");
        out.println(UtilityTags.getResource(
          "label.tags.template.trova.trovaCaseSensitive", null, true));
        out.println("</div>");
      } else {
        out.print("<input ");
      ...
      if (valVisualizzazioneAvanzata == null || valVisualizzazioneAvanzata.length() == 0) valVisualizzazioneAvanzata = "0";
      ...
      out.print(UtilityTags.getHtmlHideInput(CAMPO_COUNT,
          String.valueOf(this.getNumCampi())));
      out.println("				<SPAN id=\"contenitore-comandi-dettaglio\">");
      out.println("					<INPUT type=\"button\" class=\"bottone-azione\" value=\"Trova\" title=\"Trova\" onclick=\"javascript:trovaEsegui();\">");
      out.println("					<INPUT type=\"button\" class=\"bottone-azione\" value=\"Reimposta\" title=\"Reimposta\" onclick=\"javascript:trovaNuova();\">");
      out.println("				</SPAN>");
