 ...
 */
public abstract class AbstractGestoreCampoTabellatoArc extends
    AbstractGestoreCampo {

  /** Logger */
  static Logger   logger = Logger.getLogger(AbstractGestoreCampoTabellatoArc.class);
   ...
   */
  public AbstractGestoreCampoTabellatoArc(boolean addCodADescr, String tipoCampo) {
    ...
    if (select != null
      ...
      try {
        ...
        for (int i = 0; i < ret.size(); i++) {
          ...
          String descr = row.get(row.size() - 2).toString();
          String arc = row.get(row.size() - 1).toString();
          if (this.addCodADescr) descr = cod + " " + descr;
          this.getCampo().addValore(cod, descr, arc);
        }
