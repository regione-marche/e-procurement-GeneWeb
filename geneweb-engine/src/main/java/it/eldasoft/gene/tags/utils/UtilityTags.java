...
public class UtilityTags {
  ...
  /**
   * Path di default indicante dov si trovano tutte le pagine JSP
   * dell'applicativo
   */
  public static final String DEFAULT_PATH_PAGINE_JSP           = CostantiGenerali.PATH_WEBINF
                                                                   + "pages/";
  ...
  /**
   * Path della jsp in cui bisogna ridirezionare l'apertura della nuova maschera
   * o lista
   */
  ...
  /**
   * Numero della popUp se è aperta in popUp (Questo campo viene settato via
   * Javascript)
   */
  /** Default per identificare che si tratta di una finestra popUp */
  ...
  /** Eventuale chiave tabella parent (se la lista è su una maschera a pagine ) */
  ...
  /** Campo nascosto inserito nelle pagine a lista per gestire la modalità di update */
  ...
  /**
   * Funzione che converte una stringa per immetterla in una stringa javascript
   * o in una stringa dentro un HTML
   *
   * @param stringa
   * @return
   */
  ...
  /**
   * Partendo da un'entità ritrova il path per l'apertura
   *
   * @param entita
   * @return
   */
  public static String getPathFromEntita(String entita) {
    return UtilityTags.getPathFromTab(DizionarioTabelle.getInstance().getDaNomeTabella(
        entita));
  }
  ...
  @SuppressWarnings("rawtypes")
  private static void appendScopeAttrib(StringBuffer buf,
  ...
  public static String getHtmlHideInput(String nome, Object valore) {
    StringBuffer buf = new StringBuffer();
    String lVal = "";
    if (valore != null) lVal = valore.toString();
   ...
   *
   * @param valori
   *        Elenco di oggetti che contengono i valori
   * @param divisore
   *        Carattere utilizzato per dividere le varie parti
   * @return
   */
  ...
  public static String arrayToString(Object[] valori, char divisore) {
    ...
    for (int i = 0; i < valori.length; i++) {
      String val;
      if (i > 0) buf.append(divisore);
      if (valori[i] != null)
        // Eseguo l'aggiunta dell'eliminatore alla stringa passata
        val = replaceWithEliminatore(valori[i].toString(),
            String.valueOf(divisore));
      else
        val = "";
      buf.append(val);
    }
   ...
   * @param stringa
   * @param divisore
   * @return array di stringhe, oppure stringa vuota se la stringa di input è
   *         null
   */
  public static String[] stringToArray(String stringa, char divisore) {
    ...
    if (risultato == null) risultato = new String[] { "" };
    return risultato;
  }

  /**
   * Trasforma una stringa con l'elenco dei parametri divisi da ; nel vettore
   * con l'elenco dei parametri
   *
   * @param elencoParametri
   * @return
   */
  public static Vector<JdbcParametro> stringToVectorJdbcParametro(String elencoParametri) {
    ...
    if (elencoParametri == null || elencoParametri.length() == 0) return vRet;
    ...
    if (elencoParametri.length() > 2
        && JdbcParametro.isValidType(elencoParametri.charAt(0))
        && elencoParametri.charAt(1) == ':') {
      // siamo nel caso di parametri tipizzati, allora la gestione del ";" nei
      // valori può essere gestita
      String valore = "";
      for (int i = 0; i < valori.length; i++) {
        if (valori[i].length() > 2
            && JdbcParametro.isValidType(valori[i].charAt(0))
            && valori[i].charAt(1) == ':') {
          if (valore.length() != 0) {
   ...
   * @since 1.4.0
   */
  public static JdbcParametro[] stringToArrayJdbcParametro(
   ...
   *
   * @param nome
   *        Nome in cui i caratteri speciali venvono convertiti in _
   * @param valore
   *        Valore in cui i " vangono convertiti in \"
  ...
  public static Javascript getJavascript(PageContext pageContext) {
    ...
    if (pageContext.getAttribute(UtilityTags.REQUEST_JAVASCRIPT,
        PageContext.REQUEST_SCOPE) == null)
      pageContext.setAttribute(UtilityTags.REQUEST_JAVASCRIPT,
          new Javascript(), PageContext.REQUEST_SCOPE);
    return (Javascript) pageContext.getAttribute(
  ...
  public static String getHtmlDefaultHidden(PageContext context) {
    ...
    buf.append(getHtmlHideInput(DEFAULT_HIDDEN_FORM_JSP_PATH,
        req.getServletPath()));
    // Adesso aggiungo tutti gli hidden di default
    buf.append(getHtmlHideInput(DEFAULT_HIDDEN_FORM_TO_JSP,
        context.getAttribute(DEFAULT_HIDDEN_FORM_TO_JSP,
            PageContext.REQUEST_SCOPE)));
    // Pagina attiva
    buf.append(getHtmlHideInput(DEFAULT_HIDDEN_FORM_ACTIVEPAGE, getParametro(
        context, DEFAULT_HIDDEN_FORM_ACTIVEPAGE)));
    buf.append(getHtmlHideInput(DEFAULT_HIDDEN_IS_POPUP, getParametro(context,
        DEFAULT_HIDDEN_IS_POPUP)));
    buf.append(getHtmlHideInput(DEFAULT_HIDDEN_NUMERO_POPUP, ""));
    return buf.toString();
  }

  /**
   * Crea un campo hidden per una form con il nome del parametro ed il valore
   * del parametro
   *
   * @param context
   *        PageContext
   ...
   * @return
   */
  public static String getHtmlHideInputFromParam(PageContext context,
  ...
  public static String getParametro(PageContext context, String nomeParametro) {
    if (context.getRequest() instanceof HttpServletRequest) {
      return UtilityStruts.getParametroString(
          (HttpServletRequest) context.getRequest(), nomeParametro);
    }
    return null;
  }
  ...
  public static JdbcExpression getJdbcExpression(Campo campo) {
    if (campo != null) {
      return new JdbcExpression(
          new JdbcColumn(null, campo.getNomeFisicoCampo()));
    }
    return null;
  }
   ...
   * @param where
   * @param keys
   *        Campi chiave nel modo [NOME_CAMPO]=[VALORE];+
   */
   ...
   *
   * @param context
   *        page context con i valori dei campi chiave
   * @param objWhere
   *        Oggetto in cui aggiungere l'eventuale where
   * @param where
   *        Where in cui eseguire il replace dei valori tra ##
   */
  public static void addWhereSelezionata(PageContext context,
      JdbcWhere objWhere, String where) {
    HashMap<String, JdbcParametro> map = stringParamsToHashMap(getParametro(context,
        UtilityTags.DEFAULT_HIDDEN_KEY_TABELLA), null);
   ...
   * @param sql
   *        Sql in cui eseguire il replace dei parametri
   * @param map
   *        Stringa con la mappa dei parametri
   * @return
   */
   ...
   *
   * @param sql
   *        Sql con i parametri divisi all'interno di ##
   * @param map
   *        Mappa con i parametri
   * @return
   */
  public static String replaceParametri(Vector<JdbcParametro> parameters, String sql,
      ...
      if (pos >= 0) {
        ...
        if (pos1 > 0) {
          ...
          if (map.get(campo) instanceof JdbcParametro) {
            ...
            throw new RuntimeException("Attenzione il parametro: "
                + campo
                + " non è di tipo JdbcParametro !");
        } else
          pos = 0;
      }
    } while (pos >= 0);
   ...
   * [NOME_CAMPO=]PARAMETRO[;[NOME_CAMPO=]PARAMETRO]* il parametro deve essere
   * del formato come JdbcParametro [[TIPO_PAR]:]VALORE
   *
   * @param parametri
   *        Stringa di parametri
   * @param map
   *        Mappa alla quele aggiungere i parametri. se null viene creata
   * @return
   */
  public static HashMap<String, JdbcParametro> stringParamsToHashMap(String parametri, HashMap<String, JdbcParametro> map) {
    // Se non ancora creata la creo
    if (map == null) map = new HashMap<String, JdbcParametro>();
    ...
    for (int i = 0; i < vals.length; i++) {
      ...
      if (vals[i] != null
          && vals[i].indexOf('=') > 0
          && (vals[i].length() <= 2 || vals[i].charAt(1) != ':')) {
      ...
      if (campo != null && campo.length() > 0 && campo.indexOf('.') >= 0) {
        map.put(campo.substring(campo.indexOf('.') + 1), new JdbcParametro(
            vals[i]));
      }
    }
    return map;
  ...
  public static Vector<JdbcParametro> stringParamsToVector(String parametri, Vector<JdbcParametro> vect) {
    // Se non ancora creata la creo
    if (vect == null) vect = new Vector<JdbcParametro>();
    String vals[] = stringToArray(parametri, DEFAULT_CHAR_DIVISORE);
    // Scorro tutti i parametri
    for (int i = 0; i < vals.length; i++) {
      if (vals[i] != null
  ...
  public static Object[] vectorParamToObjectArray(Vector<?> params) {
    ...
    //if (SqlManager.DATABASE_ORACLE.equalsIgnoreCase(ConfigManager.getValore(CostantiGenerali.PROP_DATABASE))) {
    //  // oracle necessita di una gestione ad hoc e mediante JDBC
    //  lobHandler = new OracleLobHandler();
    //  ((OracleLobHandler) lobHandler).setNativeJdbcExtractor(new SimpleNativeJdbcExtractor());
    //}
    ...
    for (int i = 0; i < params.size(); i++) {
      if (params.get(i) instanceof JdbcParametro) {
        ...
        if (par.getTipo() == JdbcParametro.TIPO_BINARIO) {
          ...
          try {
            ret[i] = new SqlLobValue(
                par.byteArrayOutputStreamValue().toByteArray(), lobHandler);
        ...
        if (val != null
            && val.length() > 1
            && JdbcParametro.isValidType(val.charAt(0))
            && val.charAt(1) == ':') {
  ...
  /**
   * Trasforma una stringa nel formato TABELLA.CAMPO=VALORE. in un tipo
   * DataColumn
   *
   * @param valore
   *        Stringa nel formato
  ...
  public static DataColumn getDataColumn(String valore) {
    ...
    if (valore.indexOf('=') >= 0) {
      ...
      throw new RuntimeException(
          "getDataColumn: Formato della stringa di input noon valido: "
              + valore);
  ...
  public static DataColumn[] stringKeysToColumnWithParam(String parametroString) {
    ...
    String vals[] = UtilityTags.stringToArray(parametroString,
   ...
   *
   * @param request
   *        ServletRequest da cui estrarre il modo
   * @return true Se è in modifica false altrimenti
   */

  public static boolean isInModifica(ServletRequest request) {
    String lsModo = UtilityStruts.getParametroString(
        (HttpServletRequest) request, UtilityTags.DEFAULT_HIDDEN_PARAMETRO_MODO);
    if (lsModo == null) lsModo = UtilityTags.SCHEDA_MODO_VISUALIZZA;

    return lsModo.equals(UtilityTags.SCHEDA_MODO_MODIFICA)
        || lsModo.equals(UtilityTags.SCHEDA_MODO_INSERIMENTO);
  }
    ...
    if (href != null) {
      if (href.indexOf("$CONTEXT$") >= 0) {
        ...
        if (request instanceof HttpServletRequest) {
          ...
          href = StringUtils.replace(href, "$CONTEXT$",
  ...
  public static void salvaCurrentAction(ServletRequest request) {
    getUtilityHistory(((HttpServletRequest) request).getSession()).saveCurrent(
        request);
  }
  ...
  private static void settaAttributiGeneraliTags(ServletRequest request) {
    ...
    String elencoAttributi[] = new String[] { DEFAULT_HIDDEN_FORM_JSP_PATH,
        DEFAULT_HIDDEN_FORM_TO_JSP, DEFAULT_HIDDEN_FORM_ACTIVEPAGE,
        DEFAULT_HIDDEN_KEY_TABELLA, DEFAULT_HIDDEN_NOME_TABELLA,
        DEFAULT_HIDDEN_PARAMETRI_DA_TROVA, DEFAULT_HIDDEN_FROM_DA_TROVA,
        DEFAULT_HIDDEN_WHERE_DA_TROVA,DEFAULT_HIDDEN_PARAMETRO_MODO,
        DEFAULT_HIDDEN_KEY_TABELLA_PARENT, DEFAULT_HIDDEN_NUMERO_POPUP };
    ...
    for (int i = 0; i < elencoAttributi.length; i++) {
      ...
      if (request.getParameter(elencoAttributi[i]) != null) {
        ...
        if (request.getAttribute(elencoAttributi[i]) == null) {
          request.setAttribute(elencoAttributi[i],
              request.getParameter(elencoAttributi[i]));
        }
      }
    }
    if (request instanceof HttpServletRequest) {
   ...
   * @param toFind
   *        Carattere da trovare
   * @param eliminatore
   *        Eventuale eliminatore che fa saltare il carattere
   * @param startPos
   *        Posizione di partenza
   * @return Posizione in cui è stato trovato
   */
  public static int indexOf(String stringa, char toFind, char eliminatore,
      int startPos) {
    if (startPos < 0) startPos = 0;
    if (startPos >= stringa.length()) return -1;
    int pos = startPos - 1;
    do {
      pos = stringa.indexOf(toFind, pos + 1);
      // Se il carattere precedente è un eliminatore continuo la ricerca
  ...
  public static String convStringaHREFParemeter(String val) {
    try {
      return URLEncoder.encode(val,
          ConfigManager.getValore(CONFIGURAZIONE_ENCODING));
  ...
  public static void addDefaultAttributi(ServletRequest request) {
    ...
    // Storia Modifiche:
    // Data Utente Descrizione
    // 10/11/2006 M.F. Eliminato i campi chiave perché non necessario
    // ************************************************************
    request.setAttribute(REQUEST_VAR_HISTORY_SIZE, new Integer(
        UtilityTags.getUtilityHistory(
            ((HttpServletRequest) request).getSession()).size(
   ...
   *        Chiave del resource
   * @param params
   *        Elenco dei parametri per il replace
   * @param html
   *        flag che dice se deve essere messo all'interno di un HTML. Solo per
   *        l'errore perchè se non viene trovato viene colorato di rosso
   * @return messaggio opportunamente valorizzato
   */
  public static String getResource(String key, String[] params, boolean html) {
    return UtilityTags.getResource(
        CostantiGenerali.RESOURCE_BUNDLE_APPL_GENERALE, key, params, html);
  }
   ...
   *
   * @param resource
   *        resource bundle in cui ricercare il messaggio identificato dalla key
   * @param key
   *        Chiave del resource
   * @param params
   *        Elenco dei parametri per il replace
   * @param html
   *        flag che dice se deve essere messo all'interno di un HTML. Solo per
   *        l'errore perche se non viene trovato viene colorato di rosso
   * @return messaggio opportunamente valorizzato
   */
  public static String getResource(String resource, String key,
      String[] params, boolean html) {
    String lsKey = "";
    if (key != null) lsKey = key.toString();
    ...
    try {
      ...
      StringBuffer buf = new StringBuffer("");
      if (html) buf.append("<font color=\"#ff0000\"><b>");
      buf.append(key);
      if (html) buf.append("</b> <small><i>");
      buf.append("[");
      buf.append(t.getMessage());
      buf.append("]");
      if (html) buf.append("</i></small></font>");
      return buf.toString();
    }
    return ret;
  }
   ...
   *        Oggetto da cercare
   * @param objects
   *        Arrai di oggetti in cui cercare l'elemento
   * @return
   */
  public static int indexOf(Object obj, Object[] objects) {
    if (objects != null && objects.length > 0) {
      for (int i = 0; i < objects.length; i++) {
        if (obj != null) {
          if (objects[i] != null && obj.equals(objects[i])) return i;
        } else if (objects[i] == null) return i;
      }
    }
    return -1;
  }
  ...
  public static boolean isNavigazioneDisabilitata(PageContext pageContext) {
    String navDisabilitata = getParametro(pageContext,
        CostantiGenerali.NAVIGAZIONE_DISABILITATA);
    if (navDisabilitata != null
        && navDisabilitata.equals(CostantiGenerali.DISABILITA_NAVIGAZIONE)) {
      return true;
    }
    return false;
  }
   ...
   * @return
   */
  public static boolean checkProtection(PageContext page, String key,
      boolean defaultVal) {
  ...
  public static boolean checkProtection(PageContext page, String tipo,
      String azione, String oggetto, boolean defaultVal) {
    boolean ret = false;

    GeneManager geneManager = (GeneManager) UtilitySpring.getBean(
        "geneManager", page, GeneManager.class);
    // Estraggo il profilo dell'utente
    if (geneManager != null) {
      ret = geneManager.getProfili().checkProtec(
          (String) page.getAttribute(CostantiGenerali.PROFILO_ATTIVO,
              PageContext.SESSION_SCOPE), tipo, azione, oggetto, defaultVal);
    }
    if (PlugInBase.isSviluppo())
      DebugSviluppoFunction.addProtecCheck(page, tipo
          + "."
          + azione
          + "."
          + oggetto, ret);
  ...
  public static void removeFromStack(PageContext page, Object tag) {
    if (tag != null) {
      ...
      if (page.getRequest().getAttribute(REQUEST_VAR_TAGS_STACK) instanceof Stack) {
        stack = (Stack) page.getRequest().getAttribute(REQUEST_VAR_TAGS_STACK);
        Object obj;
        if (stack.contains(tag)) while (stack.size() > 0) {
          obj = stack.pop();
          if (obj == tag) return;
        }
      }
  ...
  public static Object getFromStack(PageContext page, Class tipoobj) {
    ...
    if (page.getRequest().getAttribute(REQUEST_VAR_TAGS_STACK) instanceof Stack) {
      ...
      for (int i = stack.size() - 1; i >= 0; i--) {
        if (stack.get(i).getClass().equals(tipoobj)) return stack.get(i);
      }
    }
    return null;
  }
  ...
  public static void addToStackTagAttributes(PageContext pageContext,
      TagAttributes attribs) {
    if (attribs != null) {
      Stack<TagAttributes> stack = null;
      if (pageContext.getRequest().getAttribute(
          REQUEST_VAR_TAG_ATTRIBUTES_STACK) instanceof Stack) {
        stack = (Stack<TagAttributes>) pageContext.getRequest().getAttribute(
            REQUEST_VAR_TAG_ATTRIBUTES_STACK);
      } else {
        stack = new Stack<TagAttributes>();
        pageContext.getRequest().setAttribute(REQUEST_VAR_TAG_ATTRIBUTES_STACK,
            stack);
      }
      // Aggiungo l'oggetto allo stack
      stack.push(attribs);
   ...
   * @return
   */
  public static TagAttributes getFromStackTagAttributes(
      PageContext pageContext, String varName) {
    if (pageContext.getRequest().getAttribute(REQUEST_VAR_TAG_ATTRIBUTES_STACK) instanceof Stack) {
      @SuppressWarnings("unchecked")
      Stack<TagAttributes> stack = (Stack<TagAttributes>) pageContext.getRequest().getAttribute(
          REQUEST_VAR_TAG_ATTRIBUTES_STACK);
      for (int i = stack.size() - 1; i >= 0; i--) {
   ...
   * @param varName
   */
  public static void removeFromStackTagAttributes(PageContext pageContext,
      TagAttributes attrib) {
    ...
    if (pageContext.getRequest().getAttribute(REQUEST_VAR_TAG_ATTRIBUTES_STACK) instanceof Stack) {
      if (attrib != null) {
        ...
        Stack<TagAttributes> stack = (Stack<TagAttributes>) pageContext.getRequest().getAttribute(
            REQUEST_VAR_TAG_ATTRIBUTES_STACK);
        Object obj;
        if (stack.contains(attrib)) while (stack.size() > 0) {
          obj = stack.pop();
          if (obj == attrib) return;
        }
      }
    }
  }
   ...
   * @param pageContext
   * @param addPagina
   *        Flag per dire di aggiungere o meno l'id della pagina
  ...
  public static String getIdRequest(PageContext page, boolean addPagina) {
    StringBuffer buf = new StringBuffer("");
    String sezioni[] = new String[] { UtilityTags.REQUEST_SCHEMA,
        UtilityTags.REQUEST_ID_MASCHERA, UtilityTags.REQUEST_ID_PAGINA };
  ...
  /**
   * Testa l'iniezione di sql malevolo nel campo di input trovaAddWhere ricevuto dalle form e che viene attivamente utilizzato dalle tag
   * libraries interne per costruire le query sulla base delle condizioni di filtro indicate.
   *
   * @param sqlCondition
   *        condizione sql ricevuta come parameter nella request
   *
   * @return true se il controllo viene bypassato oppure si considera la stringa sql non pericolosa, false se individua un pattern
   *         potenzialmente pericoloso e riconducibile a sql injection
   */
  public static boolean isFreeFromSqlInjection(String sqlCondition) {
    boolean esito = true;
    ...
    if (!"1".equals(propSaltaControllo) && StringUtils.isNotBlank(sqlCondition)) {
      // se la property di configurazione non e' esistente oppure non e' impostata a 1, si effettuano tutti i controlli. Al primo controllo
      ...
      if (esito) {
        ...
        // l'uso del commento -- invalida il resto dello statement, mentre la prima parte e' composta dai caratteri digitati dall'hacker
        esito = !StringUtils.contains(sqlCondition, "--");
      }
      if (esito) {
        // controllo 2: Piggybacked queries
        // tramite il carattere ';' di concatenazione statement vengono aggiunte ulteriori query da eseguirsi dopo quella prevista dallo
        // sviluppatore
        esito = !StringUtils.contains(sqlCondition, ";");
      }
      if (esito) {
      ...
      if (esito) {
        ...
        // iniezione di codice della forma 'OR 1==1' per rendere sempre vere delle condizioni e bypassare dei controlli
        String[] elementi = StringUtils.split(sqlCondition, ' ');
        int i = 0;
        while (i < elementi.length && esito) {
          // ciclo se ci sono ancora elementi da controllare e non ho ancora trovato una tautologia
          if ("=".equals(elementi[i]) && (i-1>=0) && (i+1<elementi.length)) {
            // se i due termini dell'uguaglianza (prima e dopo dell'uguale) sono identici ho una tautologia
            if (elementi[i-1].equals(elementi[i+1])) {
              esito = false;
            }
          } else if (StringUtils.contains(elementi[i], '=')) {
            // se e' una espressione di uguaglianza senza spazi controllo i due elementi a sx e dx dell'operatore
      ...
      if (esito) {
        // controllo 5: si bloccano comandi contenenti la lettura della versione, potrebbero essere usati per comprendere la versione esatta
        // del DBMS sottostante per attaccarlo successivamente
        esito = esito
            && !StringUtils.contains(sqlCondition, "V$VERSION")
